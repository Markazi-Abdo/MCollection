...
export const useCartStore = create((set, get) => ({
    ...
    addToCart:async function(product){
        ...
        const { cartItems } = get();
        try {
            const add = await AxiosInstance.put("/cart/put", { productId:product._id })
            set((prev)=>{
                const existInCart = prev?.cartItems?.find(item => item?._id === product._id);
                const newCart = existInCart
                ? prev.cartItems?.map(item => item?._id === product._id ? {...item, quantity:item?.quantity+1} : item)
    ...
    updateQuantity:async function(id, quantity){
        ...
        try {
            ...
            set((prev)=>{
                const findItem = prev.cartItems.find(item => item?._id === id);
                const newCart = findItem && prev.cartItems.map(item => item?._id === id ? {...item, quantity } : item)
    ...
    removeFromCart:async function(productId){
        ...
        try {
            ...
            set((prev)=>{
                const findItem = prev.cartItems.find(item => item?._id === productId);
                const newCart = findItem && prev.cartItems.filter(item => item._id !== productId);
    ...
    createSession:async function(products, coupon){
        try{
            const session = await AxiosInstance.post("/payment/session", { products, 
